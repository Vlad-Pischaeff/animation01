<script>
	...
	footer.onload = function() { 
		//...рисуем лафет
		ctx_foot.drawImage(footer, 3, 151, 40, 30);
		//...рисуем поверхность
		ctx_foot.beginPath();
		ctx_foot.moveTo(3, 180);
		ctx_foot.lineTo(397, 180);
		ctx_foot.stroke();
	};
	...
	tube.onload = function() {
		//...рисуем ствол пушки 
		ctx_tube.drawImage(tube, 10, 140, 40, 30);
	...
	const ballFire = async () => {
		...
		do {
			...
			ball.velocity.y += (2 * Math.sin(radian)) + 0.0001 * repeat * repeat;
			await sleep(20);
			repeat++;
...
<div class="container">
	<canvas id="Canvas_help" width="400" height="200" bind:this={canvas_help}></canvas>
	...
	<canvas id="Canvas_foot" width="400" height="200" bind:this={canvas_foot}></canvas>
