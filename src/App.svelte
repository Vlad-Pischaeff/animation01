<script>
	...
	let canvas_ball, canvas_tube, canvas_bg; 
	let ctx_ball, ctx_tube, ctx_bg;
	let eTube, eFooter;

	function Element(ctx, src, x, y, width, height) {
		this.ctx = ctx;
		this.src = src;
		this.x = x;
		this.y = y;
		...
		this.width = width;
		this.draw = function() {
			this.ctx.translate(this.x + 16, this.y + this.height/2);
			this.ctx.drawImage(this.src, -16, -this.height/2, this.width, this.height);
		};
		this.rotate = function(grad) {
			this.ctx.rotate(grad * Math.PI/180);
			this.ctx.drawImage(this.src, -16, -this.height/2, this.width, this.height);
		}
	}

	var footer = new Image();
	footer.onload = function() { eFooter = new Element(ctx_bg, footer, 3, 151, 40, 30) };
...
<div class="container">
	<canvas id="Canvas_ball" width="400" height="200" bind:this={canvas_ball}></canvas>
	<canvas id="Canvas_tube" width="400" height="200" bind:this={canvas_tube}></canvas>
